...
export async function GET(request: NextRequest) {
  try {
    ...
    const docs = await db
      .collection('all_company_prep_progress')
      .find({ user_id: userId })
      .toArray();
    ...
    console.error('GET /api/all-company-prep error:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const db = await getDb();
    const collection = db.collection('all_company_prep_progress');
    ...
    console.error('PUT /api/all-company-prep error:', error);
    return NextResponse.json({ error: 'server error' }, { status: 500 });
  }
}
