...
export async function GET(request: NextRequest) {
  try {
    ...
    const docs = await db
      .collection('amazon_tweak_prep_progress')
      .find({ user_id: userId })
      .toArray();
    ...
    console.error('GET /api/amazon-tweak-prep error:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const db = await getDb();
    const collection = db.collection('amazon_tweak_prep_progress');
    ...
    console.error('PUT /api/amazon-tweak-prep error:', error);
    return NextResponse.json({ error: 'server error' }, { status: 500 });
  }
}
