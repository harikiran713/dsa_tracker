...
export async function GET(request: NextRequest) {
  try {
    ...
    const docs = await db.collection('lld_progress').find({ user_id: userId }).toArray();
    ...
    console.error('GET /api/lld error:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const db = await getDb();
    const collection = db.collection('lld_progress');
    ...
    console.error('PUT /api/lld error:', error);
    return NextResponse.json({ error: 'server error' }, { status: 500 });
  }
}
