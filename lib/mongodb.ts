...
async function ensureIndexes(db: Db): Promise<void> {
  ...
  try {
    await Promise.all([
      ...
      db.collection('amazon_prep_progress').createIndex(
        { user_id: 1, leetcode_id: 1 },
        { unique: true }
      ),
      db.collection('amazon_tweak_prep_progress').createIndex(
        { user_id: 1, leetcode_id: 1 },
        { unique: true }
      ),
      ...
      db.collection('amazon_prep_progress').createIndex({ notes: 'text' }),
      db.collection('amazon_tweak_prep_progress').createIndex({ notes: 'text' }),
    ]);
    indexesEnsured = true;
  } catch (error) {
